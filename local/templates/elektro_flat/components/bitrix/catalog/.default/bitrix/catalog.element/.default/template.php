...
<div id="<?=$arItemIDs['ID']?>" class="catalog-detail-element" itemscope itemtype="http://schema.org/Product">
															...
															if($arOffer["CATALOG_QUANTITY_TRACE"] == "Y"):
																if(in_array("PRODUCT_QUANTITY", $arSetting["GENERAL_SETTINGS"]["VALUE"])):
																	...
																	if ($arOffer["CATALOG_QUANTITY"] == 0) {
																	    $availClass = '0';
																	} else if($arOffer["CATALOG_QUANTITY"] == 1 ) {
																	    $availClass = '1';
																	} else if($arOffer["CATALOG_QUANTITY"] < 5) {
																	    $availClass = '2';
																	}else if($arOffer["CATALOG_QUANTITY"] < 10) {
																	    $availClass = '3';
																	}else if($arOffer["CATALOG_QUANTITY"] < 100) {
																	    $availClass = '4';
																	}else {
																	    $availClass = '0';
																	}
																	echo '<i data-toggle="tooltip" class="ico store-stock ico-stock-level-'.$availClass.'" data-original-title="" title=""></i>';
																endif;
															endif;?>
														</span>
													</div>
													...
													<span>
														...
														if($arResult["CATALOG_QUANTITY_TRACE"] == "Y"):
															if(in_array("PRODUCT_QUANTITY", $arSetting["GENERAL_SETTINGS"]["VALUE"])):
																...
																if ($arResult["TOTAL_OFFERS"]["QUANTITY"] == $arSettingsSolo['EL_AV']['EMPTY']) {
																    $availClass = '0';
																} else if($arResult["TOTAL_OFFERS"]["QUANTITY"] == $arSettingsSolo['EL_AV']['ONE']) {
																    $availClass = '1';
																} else if($arResult["TOTAL_OFFERS"]["QUANTITY"] < $arSettingsSolo['EL_AV']['TWO']) {
																    $availClass = '2';
																}else if($arResult["TOTAL_OFFERS"]["QUANTITY"] < $arSettingsSolo['EL_AV']['THREE']) {
																    $availClass = '3';
																}else if($arResult["TOTAL_OFFERS"]["QUANTITY"] < $arSettingsSolo['EL_AV']['FOUR']) {
																    $availClass = '4';
																}else {
																    $availClass = '4';
																}
											...
											<span>
												...
												if($arResult["CATALOG_QUANTITY_TRACE"] == "Y"):
													if(in_array("PRODUCT_QUANTITY", $arSetting["GENERAL_SETTINGS"]["VALUE"])):
														...
														if ($arResult["CATALOG_QUANTITY"] == $arSettingsSolo['EL_AV']['EMPTY']) {
														    $availClass = '0';
														} else if($arResult["CATALOG_QUANTITY"] == $arSettingsSolo['EL_AV']['ONE']) {
														    $availClass = '1';
														} else if($arResult["CATALOG_QUANTITY"] < $arSettingsSolo['EL_AV']['TWO']) {
														    $availClass = '2';
														}else if($arResult["CATALOG_QUANTITY"] < $arSettingsSolo['EL_AV']['THREE']) {
														    $availClass = '3';
														}else if($arResult["CATALOG_QUANTITY"] < $arSettingsSolo['EL_AV']['FOUR']) {
														    $availClass = '4';
														}else {
														    $availClass = '4';
														}
												...
												<span>
													...
													if($arOffer["CATALOG_QUANTITY_TRACE"] == "Y"):
														if(in_array("PRODUCT_QUANTITY", $arSetting["GENERAL_SETTINGS"]["VALUE"])):
															...
															if ($arOffer["CATALOG_QUANTITY"] == $arSettingsSolo['EL_AV']['EMPTY']) {
															    $availClass = '0';
															} else if($arOffer["CATALOG_QUANTITY"] == $arSettingsSolo['EL_AV']['ONE']) {
															    $availClass = '1';
															} else if($arOffer["CATALOG_QUANTITY"] < $arSettingsSolo['EL_AV']['TWO']) {
															    $availClass = '2';
															}else if($arOffer["CATALOG_QUANTITY"] < $arSettingsSolo['EL_AV']['THREE']) {
															    $availClass = '3';
															}else if($arOffer["CATALOG_QUANTITY"] < $arSettingsSolo['EL_AV']['FOUR']) {
															    $availClass = '4';
															}else {
															    $availClass = '4';
															}
	...
	<div class="tabs-wrap tabs-catalog-detail">
		...
		<div class="tabs__box">
			...
			<?//OFFERS_DETAIL_PROPERTIES//?>
			<div class="catalog-detail" id="<?=$arItemIDs['PROPERTIES']?>">
				<?$sPropOffers = array();
				if(isset($arResult["OFFERS"]) && !empty($arResult["OFFERS"]) && $arSetting["OFFERS_VIEW"]["VALUE"] != "LIST"):
					foreach($arResult["OFFERS"] as $key => $arOffer):
						foreach($arOffer["DISPLAY_PROPERTIES"] as $propOffer):
