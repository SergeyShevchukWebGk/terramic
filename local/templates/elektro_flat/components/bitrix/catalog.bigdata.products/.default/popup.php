...
$arSettingBigdata = unserialize(gzuncompress(stripslashes(base64_decode(strtr($params["SETTINGS"], "-_,", "+/=")))));
global $arSetting;
...
<script type="text/javascript">
	BX.ready(function() {
		<?//OFFERS_LIST_PROPS//
		if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"]) && $arSettingBigdata["OFFERS_VIEW"] == "LIST"):
...
<div id="<?=$strMainID?>_info" class="item_info">	
	...
	<div class="item_block<?=($arSettingBigdata['REFERENCE_PRICE'] == 'Y' && !empty($arSettingBigdata['REFERENCE_PRICE_COEF']) ? ' reference' : '').(isset($arElement['OFFERS']) && !empty($arElement['OFFERS']) && $arSettingBigdata['OFFERS_VIEW'] == 'LIST' ? ' offers-list' : '');?>">
		<?//OFFERS_PROPS//
		if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"]) && $arSettingBigdata["OFFERS_VIEW"] != "LIST"):
			$arSkuProps = array();?>
												...
												if($arPrice["CAN_ACCESS"]):
													...
													if($price["HIDE_ZERO"] == "Y"):
														if($arSettingBigdata["REFERENCE_PRICE"] == "Y" && !empty($arSettingBigdata["REFERENCE_PRICE_COEF"])):
															if(round($arPrice["DISCOUNT_VALUE"] * $arSettingBigdata["REFERENCE_PRICE_COEF"], $price["DECIMALS"]) == round($arPrice["DISCOUNT_VALUE"] * $arSettingBigdata["REFERENCE_PRICE_COEF"], 0)):
														...
														<span class="catalog-item-price">
															...
															<?if($arSettingBigdata["REFERENCE_PRICE"] == "Y" && !empty($arSettingBigdata["REFERENCE_PRICE_COEF"])):?>
																<span class="catalog-item-price-reference">
																	<?=number_format($arPrice["DISCOUNT_VALUE"] * $arSettingBigdata["REFERENCE_PRICE_COEF"], $price["REFERENCE_DECIMALS"], $price["DEC_POINT"], $price["THOUSANDS_SEP"]);?>
																	<span><?=$currency?></span>
																</span>
															<?endif;?>
														</span>
...
<?if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])):
	foreach($arElement["OFFERS"] as $key => $arOffer):		
		...
		<?elseif($arOffer["CAN_BUY"] && $arOffer["MIN_PRICE"]["DISCOUNT_VALUE"] > 0 && $arSettingBigdata["OFFERS_VIEW"] == "LIST"):
			//BUY_ONE_CLICK//
			$properties = array();
			if(!empty($arOffer["PROPERTIES"]["ARTNUMBER"]["VALUE"])):		
				$properties[] = array(
...
if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])):
	$arJSParams = array(
		...
		"OFFERS_VIEW" => $arSettingBigdata["OFFERS_VIEW"],
